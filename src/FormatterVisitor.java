...
public class FormatterVisitor extends SysYParserBaseVisitor<String>
{
    ...
    public String visitStmt(SysYParser.StmtContext ctx)
    {
        ...
        else if (ctx.getText().startsWith("if"))
        {
            ...
            { // 存在 else 部分
                s.append("\n" + indent() + "else");
                String stmt1 = visit(ctx.stmt(1));
                if (!stmt1.trim().startsWith("{") && !stmt1.trim().startsWith("if"))
                {
                    indentLevel++;
                    stmt1 = "\n" + indent() + stmt1;
                    indentLevel--;
