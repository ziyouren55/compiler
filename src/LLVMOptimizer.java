...
//                propagateConstants(func);
    ...
    private void propagateConstants(LLVMValueRef func) {
        ...
        for (Map.Entry<LLVMBasicBlockRef, List<LLVMBasicBlockRef>> entry : conditionalBranches.entrySet()) {
            for (LLVMBasicBlockRef branchBlock : entry.getValue()) {
                ...
                for (LLVMValueRef inst = LLVMGetFirstInstruction(
                        branchBlock); inst != null; inst = LLVMGetNextInstruction(inst)) {
                    int opcode = LLVMGetInstructionOpcode(inst);
                    if (opcode == LLVMStore) {
    ...
    private void fixGlobalVariableUpdates(LLVMValueRef func) {
        ...
        // System.out.println("开始检查函数 " + LLVMGetValueName(func).getString() + "
        // 的全局变量更新");
    ...
    /**
     * 消除冗余的加载操作
     * 当两个load指令加载同一内存位置且中间没有store操作时，可以消除第二个load
     */
    private void eliminateRedundantLoads(LLVMValueRef func) {
        // 追踪每个内存位置的最后一次加载指令
        Map<String, LLVMValueRef> lastLoads = new HashMap<>();

        // 需要替换的load指令及其对应的原始load结果
        Map<LLVMValueRef, LLVMValueRef> replacements = new HashMap<>();

        // 按基本块遍历，每个基本块内部进行优化
        for (LLVMBasicBlockRef bb = LLVMGetFirstBasicBlock(func); bb != null; bb = LLVMGetNextBasicBlock(bb)) {
            // 每个基本块开始时重置lastLoads (保守起见，不跨基本块优化)
            lastLoads.clear();
            ...
            for (LLVMValueRef inst = LLVMGetFirstInstruction(bb); inst != null; inst = LLVMGetNextInstruction(inst)) {
                ...
                if (opcode == LLVMLoad) {
                    LLVMValueRef ptr = LLVMGetOperand(inst, 0); // 加载的内存位置
                    String ptrName = LLVMGetValueName(ptr).getString();
                    ...
                    // 检查是否已经有对同一位置的加载
                    if (lastLoads.containsKey(ptrName)) {
                        // 找到冗余加载，标记为可替换
                        replacements.put(inst, lastLoads.get(ptrName));
                    } else {
                        // 记录这次加载
                        lastLoads.put(ptrName, inst);
                    }
                } else if (opcode == LLVMStore) {
                    // 如果有store操作，需要清除对应内存位置的lastLoad记录
                    LLVMValueRef ptr = LLVMGetOperand(inst, 1); // 存储的目标位置
                    String ptrName = LLVMGetValueName(ptr).getString();
                    ...
                    // 移除这个内存位置的加载记录
                    lastLoads.remove(ptrName);
                } else if (opcode == LLVMCall) {
                    // 保守处理：函数调用可能修改任何全局变量
                    // 清除所有以@开头的记录(全局变量)
                    lastLoads.entrySet().removeIf(entry -> entry.getKey().startsWith("@"));
                }
            }
        }
        ...
        // 执行替换
        for (Map.Entry<LLVMValueRef, LLVMValueRef> entry : replacements.entrySet()) {
            LLVMValueRef redundantLoad = entry.getKey();
            LLVMValueRef originalLoad = entry.getValue();

            // 用第一次加载的结果替换冗余加载
            LLVMReplaceAllUsesWith(redundantLoad, originalLoad);

            // 删除冗余的load指令
            LLVMInstructionEraseFromParent(redundantLoad);
        }
